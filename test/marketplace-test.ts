...
describe("Marketplace", function () {
  ...
  const baseTokenURI: string = "TEST";
  ...
  describe("Checking setters", () => {
    ...
    it("Check function setERC20Token()", async () => {
    ...
    it("Check function setNFTAddress()", async () => {
  ...
  describe("Cheking token status", () => {
    ...
    it("Check getTokenStatus(1) == ONAUCTION", async () => {
      await Market.createItem(baseTokenURI, signers[0].address);
  ...
  describe("Checking auction & sell status", () => {
    ...
    it("Check getAuctionOrder()", async () => {
      await Market.createItem(baseTokenURI, signers[0].address);
      await Market.listItemOnAuction(1, defaultPrice);
  ...
  describe("BuyItem", () => {
    ...
    it("Should buyer spend tokens & receive the NFT", async () => {
      await Market.listItem(1, defaultPrice);
    ...
    it("Should reverted because sale status not an active ", async () => {
      await expect(
        Market.connect(signers[1]).buyItem(1))
        .to.be.revertedWith("The token isn't on sale");
    });

    it("Should sold items amount incremented ", async () => {
      await Market.listItem(1, defaultPrice);
    ...
    it("Should sale status is sold", async () => {
      await Market.listItem(1, defaultPrice);
      await Market.connect(signers[1]).buyItem(1);
  ...
  describe("Should cancel", () => {
    ...
    it("Should order status is CANCELED", async () => {
      await Market.createItem(baseTokenURI, signers[0].address);
      await Market.listItem(1, defaultPrice);
    ...
    it("Should Token status is ACTIVE", async () => {
      await Market.createItem(baseTokenURI, signers[0].address);
      await Market.listItem(1, defaultPrice);
